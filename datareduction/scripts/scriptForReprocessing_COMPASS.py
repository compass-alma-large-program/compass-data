...
make_repBW_images = False                # generate images corresponding to the requested representative bandwidth
mitigate = False                         # run hif_checkproductsize() and mitigate created products if necessary. 
...
field = 'BHR71-IRS1'                            # String specifying fields to be imaged; default is all (pending mitigation)
                                        #     Example: ’3C279, M82’
...
hm_imsize = [500,500]                   # Image x and y size in pixels or PB level; default is automatically determined
                                        #     Examples: ’0.3pb’, [120, 120]
hm_cell = '0.064arcsec'                 # Image cell size; default is automatically determined
...
robust = 0.5                            # Robust value to image with; default is automatically determined
                                        #     Example: 0.5
...
if not any([args.calibrated_final, args.calibrated_final_line, args.contsub, args.contsub_fast, args.image, args.cleanup, args.weblog]):
    print("ERROR: at least one argument must be selected")
    parser.print_help()
    sys.exit()

if args.contsub and args.contsub_fast:
    print("ERROR: please select either --contsub OR --contsub_fast, not both")
    sys.exit()

if (args.contsub or args.contsub_fast) and glob.glob("measurement_sets/uid*targets_line.ms"):
...
if args.weblog and args.contsub_fast:
    print("ERROR: --contsub_fast does not produce a weblog, as no pipeline commands are used. Please use --contsub instead, or --weblog separately to view the calibration and imaging weblog")
    sys.exit()
...
if args.calibrated_final and (args.contsub or args.contsub_fast):
    print("NOTE: Both calibrated_final.ms and uid*targets_line.ms will be created. calibrated_final.ms will not have any continuum subtracted data within it. Use --calibrated_final_line to concatenate the uid*targets_line.ms produced by --contsub or --contsub_fast.")

cleanup_only = False
if args.cleanup and not any([args.calibrated_final, args.calibrated_final_line, args.contsub, args.contsub_fast, args.image]):
    cleanup_only = True
...
# If contsub_fast, we need *uvcont.tbl and the measurement sets
if args.contsub_fast:
    os.system("cp -rf ../caltables/*uvcont.tbl .")
    os.system("mv ../measurement_sets/uid*targets.ms .")
    ms_moved = True
...
if args.contsub_fast:
    ...
    # Subtract the continuum using the delivered *uvcont.tbl tables for all spws all fields
    ASDM_list = glob.glob("uid*_targets.ms")
    for ASDM in ASDM_list:
        uvcont_tables = glob.glob(ASDM+"*uvcont.tbl")
        for uvcont_tbl in uvcont_tables:
            field_str = ".".join("_".join(uvcont_tbl.split("_targets.ms.hif_uvcontfit.")[1].split(".uvcont.tbl")[0].split("_")[1:]).split(".")[1:])

            # do the applycal per field
            applycal(vis=ASDM, field='"'+field_str+'"',
                intent='OBSERVE_TARGET#ON_SOURCE', antenna='*&*',
                gaintable=uvcont_tbl)

        # do the mstransform per ASDM
        mstransform(vis=ASDM,
            outputvis=ASDM[:-3] + '_line.ms',
...
if args.image:
    ...
    if not glob.glob("uid*targets_line.ms") and (make_cube_images or make_repBW_images):
        ...
        print("WARNING: you are going to  image cubes without contsub being run first. Proceeding with caution.")
        pass
    #else:
    if make_cube_images:
            if robust:
...
if args.calibrated_final_line:
    ...
    if not list_of_ms_line:
        print("ERROR: No uid*targets_line.ms files present to concatenate and create calibrated_final_line.ms. --contsub or --contsub_fast must be run first.")
        sys.exit()
    else:
